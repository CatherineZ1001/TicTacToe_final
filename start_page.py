...
def start_page(screen):
    ...
    while True:
        ...
        x_settings_text = settings_button.x + (settings_button.width - settings_text.get_width()) // 2
        y_settings_text = settings_button.y + (settings_button.height - settings_text.get_height()) // 2
        #place settings button
        screen.blit(settings_text, (x_settings_text, y_settings_text))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if start_button.collidepoint(event.pos): #clic the start button to start game
                    return "START_GAME"
